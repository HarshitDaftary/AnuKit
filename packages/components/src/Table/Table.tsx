...
// Re-export hook types for external consumers via this module
...
const Table = <T,>({
  ...
  const getColumnClasses = useCallback((column: ColumnDef<T>, base: string, extra: string[] = []) => {
  ...
  const handleRowClick = useCallback((row: T, index: number) => onRowClick?.(row, index), [onRowClick]);
