...
function optimizeComponent(componentName) {
  ...
  accessPatterns.forEach(pattern => {
    ...
    const newMatches = content.match(new RegExp(pattern.replace('\\\\[', '\\\\[encodeSizeMode\\\\('), 'g'));
    if (newMatches) replacements += newMatches.length;
  });
  
  // Pattern 3: Direct string comparisons in conditionals
  objectKeyPatterns.forEach(sizeMode => {
    const comparePattern = new RegExp(`([a-zA-Z]+)\\s*===\\s*['"]${sizeMode}['"]`, 'g');
    const matches = content.match(comparePattern);
    if (matches) {
      content = content.replace(comparePattern, `encodeSizeMode($1) === encodeSizeMode('${sizeMode}')`);
      replacements += matches.length;
    }
  });
...
// High-impact components (21-15 size modes)
const highImpactComponents = [
  'Progress',    // 21 size modes
  'Select',      // 19 size modes  
  'Textarea',    // 19 size modes
  'Card',        // 18 size modes
  'Input',       // 17 size modes
  'Badge',       // 16 size modes
  'TextField',   // 15 size modes
  'Avatar'       // 15 size modes
];

console.log('🚀 Applying Size Mode Encoding - High Impact Components\\n');

let totalReplacements = 0;
let optimizedComponents = 0;
let errors = [];

highImpactComponents.forEach(comp => {
...
console.log(`📊 High Impact Optimization Summary:`);
console.log(`  Components Processed: ${optimizedComponents}/${highImpactComponents.length}`);
console.log(`  Total Encodings Applied: ${totalReplacements}`);
console.log(`  Estimated Savings: ~${Math.round(totalReplacements * 3)}KB`);
...
console.log(`\\n🎯 High-impact size mode encoding complete!`);